...
//    }
    ...
    private void sendTx(String depAddr) {
        try {
            signed_transaction tx = BtsManager.getInstance().transfer(walletManager.getWalletFriendlyAddress(), depAddr, String.valueOf(baseAmount), "BTS", "", walletManager.getWalletFriendlyAddress(), Coders.decodeBase64(sharedManager.getLastSyncedBlock()));
            if (tx != null) {
                closeProgress();
            ...
            closeProgress();
            e.printStackTrace();
            Log.d("psd", "SellConfirmCoinifyActivity sendTx: " + e.getMessage());
            Toast.makeText(getApplicationContext(), "Error while sending transaction", Toast.LENGTH_SHORT).show();
        }
    }
