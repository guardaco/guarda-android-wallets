...
//    private BigDecimal balance = new BigDecimal(0);
    ...
    private void initFeeField() {
        Coin defaultFee = Coin.valueOf(10420000);
...
//        }
        ...
        long sumSatoshi = Coin.parseCoin(etSumSend.getText().toString()).getValue();
        if (isInclude)
            arrivalAmountToSend = Coin.valueOf(sumSatoshi - currentFeeEth).toPlainString();
        else
        ...
        try {
            ...
            if (!amount.isEmpty()) {
                if (!isAmountMoreBalance(amount)) {
                    ...
                    signed_transaction tx = BtsManager.getInstance().transfer(walletManager.getWalletFriendlyAddress(), getToAddress(), getAmountToSend(), "BTS", "", walletManager.getWalletFriendlyAddress(), Coders.decodeBase64(sharedManager.getLastSyncedBlock()));
                    if (tx != null) {
                        closeProgress();
