...
public class TransactionHistoryFragment extends BaseFragment {
    ...
    private void loadTransactions() {
        ...
        final List<operation_history_object> hist = BtsManager.getInstance().getTransactions(walletManager.getWalletFriendlyAddress());
        BtsManager.getInstance().getLatestBlockHeightFromExplorer(new Callback<Integer>() {
            @Override
            public void onResponse(final Integer head_block_number) {
                try {
                        ...
                        public void run() {
                            try {
                                if (hist == null) {
                                    ...
                                    List<TransactionItem> txItems = transactionsManager.transformTxToFriendlyNew(hist, walletManager.getWalletSystemAddress(), head_block_number);
    ...
    private void loadBalance() {
        long balance = BtsManager.getInstance().getBalance(walletManager.getWalletFriendlyAddress());
        loaderAnimation.cancel();
        if (balance < 0) {
            ((MainActivity) getActivity()).showCustomToast(getString(R.string.err_get_balance), R.drawable.err_balance);
        } else {
            walletManager.setMyBalance(balance*1000);
            walletManager.setBalance(balance*1000);
    ...
    private void checkFromRestore() {
        ...
        if (args != null) {
            ...
            if (!TextUtils.isEmpty(key)) {
                ...
                if (key.length() > 4 && key.substring(0, 4).equalsIgnoreCase("xprv")) {
