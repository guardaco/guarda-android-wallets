...
public class RestoreFromBackupActivity extends AToolbarActivity {
    ...
    @BindView(R.id.tl_wallet_email)
    TextInputLayout tlWalletEmail;
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        tlWalletEmail.setVisibility(View.VISIBLE);
        ...
        if (!TextUtils.isEmpty(etBackupPhrase.getText().toString().trim())) {
            if (walletManager.isValidPrivateKey(etBackupPhrase.getText().toString())) {
                ...
                AsyncTask.execute(new Runnable() {
                    @Override
                    public void run() {
                        walletManager.restoreFromBlock2(etWalletEmail.getText().toString(), etBackupPhrase.getText().toString(), new Runnable() {
                            @Override
                            public void run() {
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        String checkPrivKey = Coders.decodeBase64(sharedManager.getLastSyncedBlock());
                                        if (!"".equals(checkPrivKey)) {
                                            goToMainActivity(etBackupPhrase.getText().toString());
                                            btnRestore.setEnabled(false);
                                        } else {
                                            showError(etWalletEmail, "*");
                                            showError(etBackupPhrase, getString(R.string.et_error_wrong_private_key_or_email));
