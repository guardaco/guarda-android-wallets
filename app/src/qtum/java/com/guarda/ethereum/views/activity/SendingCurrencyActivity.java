...
//    private BigDecimal balance = new BigDecimal(0);
    ...
    private WalletManager.UnspentOutputList unspentOutputList = new WalletManager.UnspentOutputList();
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        initSendSumField();
        initFeeField();
        final SendingCurrencyActivity thisActivity = this;
        walletManager.getUnspentOutput(new Callback<WalletManager.UnspentOutputList>() {
            @Override
            public void onResponse(WalletManager.UnspentOutputList response) {
                unspentOutputList = response;
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (unspentOutputList.list == null) {
                            Toast.makeText(thisActivity, getString(R.string.exchange_error_internet_connection), Toast.LENGTH_SHORT).show();
                        } else {
                            updateArrivalField();
                            updateWarnings();
                            updateArrivalField();
    ...
    private void initFeeField() {
        Coin defaultFee = Coin.valueOf(800000);
...
//        return amountDecimal.compareTo(feeDecimal) > 0;
    ...
    private void updateArrivalField() {
        ...
        try {
            ...
            Transaction tx = walletManager.generateTx(getToAddress(), sumSatoshi, currentFeeEth/100, unspentOutputList);
            long totalFee = (long)Math.ceil(tx.unsafeBitcoinSerialize().length * currentFeeEth / 1024.0);
        ...
        if (makeSecondCheck) {
            try {
                long sumSatoshi = Coin.parseCoin(etSumSend.getText().toString()).getValue();
                long amountSatoshi = Coin.parseCoin(getAmountToSend()).getValue();
                long totalFee = walletManager.calculateFee(getToAddress(), amountSatoshi, estimateFee, unspentOutputList, currentFeeEth);
        ...
        try {
            ...
            if (!amount.isEmpty()) {
                if (!isAmountMoreBalance(amount)) {
                    ...
                    Transaction txtest = walletManager.generateTx(getToAddress(), amountSatoshi, currentFeeEth/100, unspentOutputList);
                    long extimateFee0 = (long)Math.ceil(txtest.unsafeBitcoinSerialize().length * currentFeeEth / 1024.0);
                    long estimateFee = walletManager.calculateFee(getToAddress(), amountSatoshi, extimateFee0, unspentOutputList, currentFeeEth);
                    Log.d("flint", "send... estimateFee: " + estimateFee);
                    String hex = walletManager.generateHexTx(getToAddress(), amountSatoshi, estimateFee, unspentOutputList);
