...
public class SellConfirmCoinifyActivity extends AToolbarActivity {
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        final SellConfirmCoinifyActivity thisActivity = this;
    ...
    private void sendTx(String depAddr, BigDecimal fee) {
        try {
            long amountSatoshi = Coin.parseCoin(String.valueOf(baseAmount)).getValue();
            long feeSatoshi = Coin.parseCoin(fee.toPlainString()).getValue();
            Log.d("svcom", "fee = " + feeSatoshi);
            Transaction txtest = walletManager.generateTx(depAddr, amountSatoshi, feeSatoshi/100, unspentOutputList);
            long extimateFee0 = (long)Math.ceil(txtest.unsafeBitcoinSerialize().length * feeSatoshi / 1024.0);
            long estimateFee = walletManager.calculateFee(depAddr, amountSatoshi, extimateFee0, unspentOutputList, feeSatoshi);
            Log.d("flint", "send... estimateFee: " + estimateFee);
            String hex = walletManager.generateHexTx(depAddr, amountSatoshi, estimateFee, unspentOutputList);
