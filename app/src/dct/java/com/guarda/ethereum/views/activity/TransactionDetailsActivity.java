...
public class TransactionDetailsActivity extends AToolbarMenuActivity {
    ...
    private void initRepeatButton() {
        if (isDebit(walletManager.getWalletFriendlyAddress(), walletManager.getPublicKey(), transaction.getTo())) {
            btnRepeat.setVisibility(View.GONE);
        } else if (!isDebit(walletManager.getWalletFriendlyAddress(), walletManager.getPublicKey(), transaction.getFrom())) {
            btnRepeat.setVisibility(View.GONE);
        } else {
    ...
    private void setValue(String respValue) {
        ...
        String valueStr = (isDebit(walletManager.getWalletFriendlyAddress(), walletManager.getPublicKey(), transaction.getTo()) ? "+" : "-")
                + " " + decimalFormat.format(formatted);
        etTrValue.setText(String.format("%s %s", valueStr, Common.MAIN_CURRENCY.toUpperCase()));
    }

    private boolean isDebit(String ourAddress, String ourAddress2, String toAddress) {
        try {
            return ourAddress.equals(toAddress) || ourAddress2.equals(toAddress);
        } catch (Exception e) {
            return false;
        }
    }
