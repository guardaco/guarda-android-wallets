...
public class SellConfirmCoinifyActivity extends AToolbarActivity {
        ...
        Requestor.getTxFee(new ApiMethods.RequestListener() {
            ...
            public void onSuccess(Object response) {
                try {
                    ...
                    sendTx(depAddr, fee.toPlainString());
                } catch (Exception e) {
                    closeProgress();
    ...
    private void sendTx(String depAddr, String fee) {
        long amountSatoshi = WalletAPI.coinsToSatoshiLong(baseAmount);

        showProgress(getString(R.string.progress_bar_sending_transaction));
        String strTo = depAddr;
        if (strTo.indexOf("@") != -1)
            strTo = "u"+ Coders.md5(strTo);
        WalletAPI.sendTransactionFromName("u"+Coders.md5(sharedManager.getWalletEmail()), Coders.decodeBase64(sharedManager.getLastSyncedBlock()), strTo, amountSatoshi, WalletAPI.coinsToSatoshiLong(fee), new Callback<Boolean>() {
            @Override
            public void onResponse(Boolean response) {
                ...
                if (response) {
                    ...
                    Toast.makeText(getApplicationContext(), getString(R.string.transaction_not_sent_fee), Toast.LENGTH_LONG).show();
                }
            }
        });
