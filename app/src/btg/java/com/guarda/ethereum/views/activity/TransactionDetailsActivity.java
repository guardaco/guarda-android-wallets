...
//    private static final String BLOCK_EXPLORER_URL = "https://explorer.bitcoingold.org/insight/tx/";
    private static final String BLOCK_EXPLORER_URL = "https://btgexplorer.com/tx/";
    ...
    private void setBalanceDetails() {
        long balanceBefore = transactionsManager.getBalanceByTime(true, walletManager.getMyBalance().getValue(), transaction);
        long balanceAfter;
        if (balanceBefore == 0) {
            balanceAfter = getStartBalanceAfter(balanceBefore, transaction.getValue());
        } else {
            balanceAfter = transactionsManager.getBalanceByTime(false, walletManager.getMyBalance().getValue(), transaction);
        }
