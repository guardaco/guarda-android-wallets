...
public class TransactionsManager {
    ...
    private boolean getIsOut(BtgTxResponse item, String ownAddress) {
        if (item.getVin().get(0).getAddr() == null) {
            ...
            for (Vin in : item.getVin()) {
                if (in.getAddr().equals(ownAddress))
                    return true;
            }
            return false;
        }
    ...
    private long calculateTxNew(BtgTxResponse item, String ownAddress) {
        ...
        if (item.getVin().get(0).getAddr() != null) {
            for (Vin in : item.getVin()) {
                if (in.getAddr().equals(ownAddress)) {
                    txSum += convertStrCoinToSatoshi(in.getValue().toString());
//                    return getOutsSumNew(item, ownAddress);
                }
            }

            if (txSum == 0)
                return getInputsSumNew(item, ownAddress);
...
//            }
    ...
    private long getInputsSumNew(BtgTxResponse item, String ownAddress) {
        ...
        for (Vout out : item.getVout()) {
            if (out.getScriptPubKey().getAddresses() == null) continue;
            if (out.getScriptPubKey().getAddresses().get(0).equals(ownAddress)) {
                 return convertStrCoinToSatoshi(out.getValue());
            }
        }
        return res;
    }
    ...
    public long getBalanceByTime(boolean balanceBefore, long curBalanceSatoshi, TransactionItem tx) {
        ...
        for (TransactionItem item : txFriendlyList) {
            if (balanceBefore) {
                ...
                //check time and if different block has the same time
                if (item.getTime() > time || (item.getTime() == time && !tx.getHash().equals(item.getHash()))) {
                    Log.d("psd", "item =" + item.getHash() +  " " + item.getTime() + " > " + time);
                    if (item.isOut()) {
                        res += item.getValue();
                        Log.d("psd", "isOut - res="+res);
                    } else {
                        res -= item.getValue();
                        Log.d("psd", "not isOut - res="+res);
                    }
                }
            }
        }
