...
public class TransactionsManager {
    ...
    public List<TransactionItem> transformTxToFriendlyNew(List<Transfer> transactions, String ownAddress) {
        txFriendlyList.clear();
        for (Transfer tx : transactions) {
            TransactionItem item = new TransactionItem();
            item.setHash(tx.getHash());
            item.setTime(tx.getTimestamp());
//            long sumTx = calculateTxNew(tx, ownAddress);
            //item.setFrom(tx.getAddress());
            item.setTo(tx.getAddress());
            item.setSum(Math.abs(tx.getValue()*100));
            if (tx.getValue() < 0)
                item.setOut(true);
//            item.setReceived(sumTx < 0);
            item.setConfirmations(tx.getPersistence() ? 1 : 0);
