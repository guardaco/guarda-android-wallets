...
public class TransactionHistoryFragment extends BaseFragment {
    ...
    private void loadBalanceAndTransactions() {
        ...
        walletManager.requestBalanceAndTransactions(Coders.decodeBase64(sharedManager.getLastSyncedBlock()), new Callback<GetAccountDataResponse>() {
            @Override
            public void onResponse(GetAccountDataResponse response) {
                try {
                    thisFragment.getActivity().runOnUiThread(() -> {
                        if (response != null) {
                            loaderAnimation.cancel();
                            balanceHasLoaded(response);
                            transactionsHasLoaded(response);
                        } else {
                            loaderAnimation.cancel();
    ...
    private void balanceHasLoaded(GetAccountDataResponse response) {
        walletManager.setMyBalance(response.getBalance()*100);
        walletManager.setBalance(response.getBalance()*100);
    ...
    private void transactionsHasLoaded(GetAccountDataResponse response) {
        List<TransactionItem> txItems = transactionsManager.transformTxToFriendlyNew(response.getTransfers(), walletManager.getWalletFriendlyAddress());
