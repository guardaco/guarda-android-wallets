...
public class SellConfirmCoinifyActivity extends AToolbarActivity {
        ...
        Requestor.coinifyTradeSell(sharedManager.getCoinifyAccessToken(), trade, new ApiMethods.RequestListener() {
            ...
            public void onSuccess(Object response) {
                if (response != null) {
                    ...
                    Log.d("psd", "PurchaseCoins coinifyTrade success - trade id = " + ctr.getId() + " coinify btc address = " + ctr.getTransferIn().getDetails().getAccount());
                }
            }
            ...
            public void onResponse(final String status, final ChangenowApi.GenerateAddressRespModel resp) {
                try {
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            if ("ok".equals(status)) {
                                getGasPrice(resp.depositAddress);
                            } else {
    ...
    private void getGasPrice(String depAddr) {
        networkManager.getGasPrice(new Callback<BigInteger>() {
            @Override
            public void onResponse(BigInteger response) {
                BigInteger gasPrice = response;
                if (gasPrice != null) {
                    getTxFee(depAddr, gasPrice);
                } else {
                    closeProgress();
                    Toast.makeText(getApplicationContext(), "Can not get gas price. Please, try later", Toast.LENGTH_SHORT).show();
                    Log.d("psd", "getGasPrice - onResponse: gasPrice is null");
                }
    ...
    private void getTxFee(String depAddr, BigInteger gasPrice) {
        Requestor.getTxFee(new ApiMethods.RequestListener() {
            ...
            public void onSuccess(Object response) {
                try {
                    ...
                    BigInteger currentFeeWei = Convert.toWei(fee, Convert.Unit.ETHER).toBigInteger();
                    BigInteger gasLimitForFeeMode = currentFeeWei.divide(gasPrice);
                    sendTx(depAddr, gasPrice, gasLimitForFeeMode);
                } catch (Exception e) {
                    closeProgress();
                    Toast.makeText(getApplicationContext(), "Can not get exchange fee. Please, try later", Toast.LENGTH_SHORT).show();
                    e.printStackTrace();
    ...
    private void sendTx(String depAddr, BigInteger gasPrice, BigInteger gasLimitForFeeMode) {
        BigInteger value = new BigDecimal(Float.toString(baseAmount)).toBigInteger();
        if (BuildConfig.DEBUG) {
            Toast.makeText(getApplicationContext(),
                    "sendTransaction() for debug",
                    Toast.LENGTH_SHORT).show();
            Log.d("psd", "sendTransaction() for debug - value = " + value + " gasPrice = " + gasPrice + " gasLimitForFeeMode = " + gasLimitForFeeMode);
            networkManager.sendTransaction("0x364f15659deef1dfb12400adc2f3f5643123b550", new BigDecimal(Float.toString(baseAmount/10000.0f)).toBigInteger(), gasPrice, gasLimitForFeeMode, "",
            ...
            networkManager.sendTransaction(depAddr, value, gasPrice, gasLimitForFeeMode, "",
                    new Callback<RawTransactionResponse>() {
                        @Override
                        public void onResponse(RawTransactionResponse response) {
                            if (response != null) {
