...
public class ZcashTransactionHelper {
    ...
    public static byte[] buildOutDesc(SaplingCustomFullKey privKey, Long value) {
        String rhex = RustAPI.genr();
        Timber.d("getUotputs rhex=%s", rhex);
        SaplingNotePlaintext snp = new SaplingNotePlaintext(privKey.getD(), TypeConvert.longToBytes(value), hexToBytes(rhex), new byte[0], rhex);

        // SaplingNoteEncryption
        String eskhex = RustAPI.genr();
        String epkstr = RustAPI.epk(Utils.bytesToHex(privKey.getD()), eskhex);
        ...
        SaplingNotePlaintextEncryptionResult snper = snp.encrypt(privKey.getPkd(), sne);
         ...
         * rcv - 32 bytes
         */
        ProofAndCv pacv = getProof(privKey, eskhex, snp.getRcmStr(), value);
        ...
        SaplingOutgoingPlaintext sop = new SaplingOutgoingPlaintext(privKey.getPkd(), snper.sne.eskbS);
    ...
    private static ProofAndCv getProof(SaplingCustomFullKey privKey, String eskhex, String rStr, Long value) {
        /**
         * expected 256 bytes (proof + cv + rcv)
         */
        Timber.d("greeting eskhex=%s s=%s", eskhex, eskhex.length());
        Timber.d("greeting d=%s s=%s", Arrays.toString(privKey.getD()), privKey.getD().length);
        Timber.d("greeting pkd=%s s=%s", Arrays.toString(privKey.getPkd()), privKey.getPkd().length);

        byte[] gstr = RustAPI.greeting(eskhex,
                Utils.bytesToHex(privKey.getD()),
                Utils.bytesToHex(privKey.getPkd()),
                rStr,
                value.toString());
