...
public class TransactionHistoryFragment extends BaseFragment {
    ...
    private static final String ZERO_BALANCE = "...";
    ...
    private void showTransactions(boolean forceUpdate) {
    ...
    private void showBalance(boolean withCache) {
        if (isAdded() && !isDetached() && isVisible && NetworkManager.isOnline(getActivity())) {
            ...
            askRating();
            tvUpdateTransactions.setClickable(false);
            RequestorBtc.getCurrentBlockHeight(String.valueOf(System.currentTimeMillis()), new ApiMethods.RequestListener() {
                @Override
                public void onSuccess(Object response) {
                    BlockHeightResponse block = (BlockHeightResponse) response;
                    loadTransactionsAndBalance(block);
                }
    ...
    private void loadTransactionsAndBalance(final BlockHeightResponse block) {
        if (walletManager.getWalletFriendlyAddress() != null) {
            RequestorBtc.getBalanceAndTx(walletManager.getWalletFriendlyAddress(), new ApiMethods.RequestListener() {
                @Override
                public void onSuccess(Object response) {
                    BalanceAndTxResponse balance = (BalanceAndTxResponse) response;
                    walletManager.setMyBalance(balance.getFinalBalance());
                    walletManager.setBalance(balance.getFinalBalance());
                    String curBalance = walletManager.getFriendlyBalance(walletManager.getMyBalance());

                    List<TransactionItem> txItems = transactionsManager.transformTxToFriendly(balance.getTransactions(),
                            block.getLastBlockHeight(), walletManager.getWalletFriendlyAddress());
